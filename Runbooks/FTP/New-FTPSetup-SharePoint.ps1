...
Try
{
    ...
    $FTPSetup = New-ADFTPSetup -Owner $AccountOwner.DistinguishedName `
                                ...
                                -Members $GroupMember.DistinguishedName `
    ...
    Send-AutomationCommunication -RequestName 'New FTP Setup' `
                                            -Type Success `
                                            -RequestLink $Request.DisplayI `
                                            -AdditionalInformation $AdditionalInformation `
                                            -To $RequesterEmail `
                                            -Cc $Vars.ErrorDistlist `
                                            -Contact $Vars.SupportContact `
    ...
    Update-SPListItem -SPUri $NewRequestURI `
                        -Data @{ 'Status' = 'Success' } `
...
Catch
{
    ...
    Switch($ExceptionInfo.FullyQualifiedErrorId)
    {
        Default
        {
            ...
            $AdditionalInformation = Format-AdditionalInformation -Exception $Exception
            Send-AutomationCommunication -RequestName 'New FTP Setup' `
                                            -Type Failure `
                                            -RequestLink $Request.DisplayId `
                                            -AdditionalInformation $AdditionalInformation `
                                            -To $RequesterEmail `
                                            -Cc $Vars.ErrorDistlist `
                                            -Contact $Vars.SupportContact `
